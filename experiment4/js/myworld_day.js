...
function createColorPalettes() {
  waterColors = [
    color(0, 150 + (worldSeed % 50), 200 + (worldSeed % 50)), // deep aqua
    color(0, 180 + (worldSeed % 40), 230 + (worldSeed % 40)), // lighter turquoise
    color(100 + (worldSeed % 30), 220 + (worldSeed % 30), 255) // bright shallow water
  ];

  skyColors = [
    color(135 + (worldSeed % 10), 206 + (worldSeed % 20), 250),  // bright blue sky top
    color(110 + (worldSeed % 10), 190 + (worldSeed % 20), 224),  // soft blue sky middle
    color(173 + (worldSeed % 10), 216 + (worldSeed % 20), 230)   // pale sky blue bottom
  ];
}
...
function p3_drawTile(i, j) {
  ...
  if (j <= skyTopThreshold) {
    ...
    if (distanceToSun < 30) {
      let sunColor = color(255, 255, 220, 255); // clean bright white-yellow sun
    ...
    if (distanceToSun < 8) {
      ...
      let sunColor = color(220, 150 + (worldSeed % 50), 50, 255);
  ...
  // Sand colors
    ...
    if (horizontalDistance < beachWidth) {
      ...
      let r = 235 + (sandSeed % 10); // Bright warm sand
      let g = 215 + (sandSeed % 10);
      let b = 180 + (sandSeed % 10);
      fill(r, g, b, 230);
    } 
